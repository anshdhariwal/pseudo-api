...
function sendprompt(text) {
  var box = document.querySelector('div[contenteditable="true"].ProseMirror');
  if (!box) return false;
  box.focus();
...
function waitforanswer(snapcount, cb) {
  ...
  var t = setInterval(function () {
    var all = document.querySelectorAll('div[data-testid="assistant-message"] .prose');
    if (all.length <= snapcount) return;

    var last = all[all.length - 1];
    var text = last.innerText.trim();

    var streaming = document.querySelector('span[data-testid="streaming-indicator"]');
...
window.addEventListener('message', function (e) {
  ...
  if (data.action === 'ASK') {
    var snap = document.querySelectorAll('div[data-testid="assistant-message"] .prose').length;
...
waitfor('div[contenteditable="true"].ProseMirror', function () {
